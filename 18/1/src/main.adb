...
begin
   ...
   begin
      Reducer : while Length (Input) > 1 loop
         declare
            Left   : constant List := Pop (Input);
            Right  : constant List := Pop (Input);
            Result : List          := Left + Right;
         begin
            Reduce (Result);
            Input.Prepend (Result, 1);
         end;
      end loop Reducer;

      Result_List := Input.First_Element;

      Magnitude : declare
         Snailfish_Number : List := Input.First_Element;
         Elt              : Integer;
         Curs,
         Temp_Curs        : Snailfish_Math_List.Cursor;
      begin
